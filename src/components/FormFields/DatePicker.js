...
export function DatePickerExample({name, label}) {
  ...
  return (
    <div className="mb-8">
      ...
      >
        <Group className="flex focus-within:bg-white group-open:bg-white pl-3 focus-visible:ring-2 px-4 py-3 w-full font-medium border rounded-lg appearance-none text-gray-900 dark:text-white  focus:outline-none focus:ring-0 transition-colors duration-300 bg-transparent focus:bg-white dark:bg-transparent dark:focus:bg-gray-950 caret-gray-600 border-gray-400 dark:border-gray-600 dark:focus:border-gray-500 focus:border-gray-800 peer">
          <DateInput className="flex flex-1 py-2">
            {(segment) => (
              <DateSegment
                segment={segment}
                className="text-gray-900 px-0.5 tabular-nums outline-none rounded-sm focus:bg-violet-700 focus:text-white caret-transparent placeholder-shown:italic"
              />
            )}
          </DateInput>
          <label
                    ...
                    return (
                      <CalendarCell
                        ...
                        className={(props) => {
                          ...
                          return `w-9 h-9 outline-none cursor-default rounded-full flex items-center justify-center outside-month:text-gray-100 pressed:bg-gray-200 selected:bg-violet-700 selected:text-white focus-visible:ring ring-violet-600/70 ring-offset-2 ${
                            isDisabled ? "text-gray-200" : "hover:bg-gray-100"
                          }`;
                        }}
                      />
                    );
